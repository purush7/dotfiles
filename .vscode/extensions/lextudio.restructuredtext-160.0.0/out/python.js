...
class Python {
    ...
    async checkSnootyInstall() {
        try {
            const versionTooOld = await this.exec('-c', '"import snooty; from distutils.version import LooseVersion; print(LooseVersion(snooty.__version__) < LooseVersion(\'1.9.1\'))"');
            return versionTooOld.trim() === 'False';
        }
        catch (e) {
            return false;
